...
for idx_file = 1:2:length(img_list)
    ...
    for idx_scale = 1:length(scale_lisht)
        ...
        %% extract patches of size 15*45 with stride 20
        idx_patch=1;
        for x_lr = 3:20:size(img_lr_0,1)-18
            for y_lr = 3:20:size(img_lr_0,2)-48
                x_hr = (x_lr-1) * scale + 1;
                y_hr = (y_lr-1) * scale + 1;
                hr_patch_0 = img_hr_0(x_hr:(x_lr+14)*scale,y_hr:(y_lr+44)*scale,:);
                hr_patch_1 = img_hr_1(x_hr:(x_lr+14)*scale,y_hr:(y_lr+44)*scale,:);
                lr_patch_0 = img_lr_0(x_lr:x_lr+14,y_lr:y_lr+44,:);
                lr_patch_1 = img_lr_1(x_lr:x_lr+14,y_lr:y_lr+44,:);
