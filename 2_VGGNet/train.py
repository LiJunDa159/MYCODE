...
logs_path='G:\\MYWORK\\2_VGGNet\\logs'
...
with tf.name_scope('Y_pred'):
    y_pred = vggnet.VGG16(x, num_classes, IS_PRETRAIN)
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        if i % int(data.train.num_examples / batch_size) == 0:
            ...
            saver.save(session, './VGGNet16-model/save_model.pd', global_step=i)  # 保存模型

    total_iterations += num_iteration
#######################################正式训练########################################
train(num_iteration=88)
####################################启动可视化###################################
# #cd G:\MYWORK\2_VGGNet
# G:
# activate tensorflow
