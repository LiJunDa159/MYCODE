...
logs_path='G:\\MYWORK\\5_DenseNet\\logs'
##############################数据load##########################
...
with tf.name_scope('Y_pred'):
    y_pred = densenet.DenseNet(x=x, nb_blocks=nb_block, filters=growth_k, training=training_flag,rate=dropout_rate).model
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        feed_dict_tr = {x: x_batch,
                        y_true: y_true_batch,training_flag:True}
        feed_dict_val = {x: x_valid_batch,
                         y_true: y_valid_batch,training_flag:False}
        ...
        if i % int(data.train.num_examples / batch_size) == 0:
            ...
            saver.save(session, './DenseNet-model/save_model.pd', global_step=i)  # 保存模型

    total_iterations += num_iteration
#######################################正式训练########################################
train(num_iteration=88)
####################################启动可视化###################################
# #cd G:\MYWORK\5_DenseNet
# G:
# activate tensorflow
