...
def cut_imgs(ave_data,sobel_data,data):
    ...
    for i in range(len(data)):
        cut_ave_data= divide_method2(ave_data[i],52,77)
        cut_ave_datas.append(cut_ave_data)
        cut_sobel_data = divide_method2(sobel_data[i],52,77)
        cut_sobel_datas.append(cut_sobel_data)
        cut_data = divide_method2(data[i],52,77)
...
def PCA(datas):
    ...
    pca = decomposition.PCA(n_components=4, copy=True, whiten=True)
        ...
        for j in range(W.shape[1]):
            for k in range(W.shape[2]):
                for l in range(W.shape[3]):
                    W[i,j,k,l]=(W[i,j,k,l]-min)/denominator
    return W
...
def z_score(W):
    ...
    W_shape=W.shape
    W=W.reshape((-1, 1))
    W = preprocessing.minmax_scale(W, feature_range=(0, 0.05), axis=0, copy=True)
    W=W.reshape((W_shape))
...
def cut_feature(datas):
    cut_datas=[]
    for i in range(len(datas)):
        cut_data=divide_method3(datas[i],86,129)
        cut_datas.append(cut_data)
...
def PCA_feature(datas,number,pca_number,save_number):
    datas=np.array(datas)
    array_size=int(datas.shape[0]/number)
    ws=[]
    for i in range(number):
        cut=cut_feature(datas[i*array_size:(i+1)*array_size,:,:,:])
...
ave_corel_data=ave_img(corel_data)
sobel_corel_data=sobel_img(corel_data)
cut_ave_corel_data, cut_sobel_corel_data, cut_corel_data=cut_imgs(ave_corel_data, sobel_corel_data, corel_data)

W1_ave=PCA(cut_ave_corel_data)
W1_sobel=PCA(cut_sobel_corel_data)
W1_data=PCA(cut_corel_data)
...
#第二层权重与第二个特征图
W2=PCA_feature(feature1_imgs,10,4,32)
# W2=z_score(W2)
feature2_imgs=myconv(feature1_imgs,W2)

#第三层权重与第二个特征图
W3=PCA_feature(feature2_imgs,20,4,64)
# W3=z_score(W3)

np.save(r"G:\PyCharm\Lunwen3\Ws_corel\W1.npy", W1)
np.save(r"G:\PyCharm\Lunwen3\Ws_corel\W2.npy", W2)
np.save(r"G:\PyCharm\Lunwen3\Ws_corel\W3.npy", W3)
