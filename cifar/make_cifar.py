...
str_4={}
for i in range(0,10):
    str_3[i]='./test_cifar10'+'/'+str(i)
for i in range(0,10):
    str_4[i]='./train_cifar10'+'/'+str(i)
...
if os.path.exists(str_2) == False:
    os.mkdir(str_2)
for i in range(0,10):
    if os.path.exists(str_3[i]) == False:
        os.mkdir(str_3[i])
    if os.path.exists(str_4[i]) == False:
        os.mkdir(str_4[i])
...
def cifar_jpg(dir_file):
    ...
    for i in range(0, 10000):
        ...
        cls_name=str(Xtr[b'labels'][i])
        picName = './train_cifar10/' + cls_name + '/'+ str(Xtr[b'labels'][i]) + '_' + str(i + (j - 1) * 10000) + '.jpg'  # Xtr['labels']为图片的标签，值范围0-9，本文中，train文件夹需要存在，并与脚本文件在同一目录下。
    ...
    for i in range(0, 10000):
        ...
        cls_name = str(Xtr[b'labels'][i])
        picName = './test_cifar10/' + cls_name + '/'+ str(testXtr[b'labels'][i]) + '_' + str(i) + '.jpg'
        cv2.imwrite(picName, img)
...
if __name__ == '__main__':
    dir_file ='G:\\Network\\cifar\\cifar-10-batches-py'
try:
    cifar_jpg(dir_file)
except:
    print('函数报错')